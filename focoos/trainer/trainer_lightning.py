...
def run_train_lightning(
    ...
    if eval_only:
        logger.info("🔍 Starting Lightning evaluation (validation only)...")
    else:
    ...
    if train_args.seed >= 0:
        logger.info(f"🎲 Random seed set to: {train_args.seed + comm.get_rank()}")
    ...
    while os.path.exists(output_dir):
        logger.warning(f"Trainer Output directory {output_dir} already exists. Creating a new one...")
