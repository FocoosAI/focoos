...
class KeypointEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        if "instances" in predictions[0]:
            self._results[self.iou_type] = self._eval_predictions(predictions, inputs)
        # Copy so the caller can do whatever with results
        return copy.deepcopy(self._results)

    def _eval_predictions(self, predictions, inputs):
        """
        ...
        coco_inputs = list(itertools.chain(*[x["annotations"] for x in inputs]))  # this is a list of DetectronDict
    ...
    def _evaluate_predictions_on_coco(
        ...
        coco_gt = create_coco(images, categories, coco_gt, self.iou_type, is_gt=True)
    ...
    def _derive_coco_results(self, coco_eval, class_names=None):
        ...
        metrics = {
            ...
            "keypoints": ["AP", "AP50", "AP75", "APs", "APm", "APl"],
        }[iou_type]
...
def create_coco(images, categories, coco_dict, iou_type, is_gt=False):
    """
    ...
    if iou_type == "bbox":
        ...
        for id, ann in enumerate(anns):
            ...
            ann["id"] = id + 1
            ann["iscrowd"] = 0 if "iscrowd" not in ann else ann["iscrowd"]
            # ann["keypoints"] = ann["keypoints"]
            ann["num_keypoints"] = sum([x > 0.5 for x in ann["keypoints"][2::3]])  # count number of visible keypoints
