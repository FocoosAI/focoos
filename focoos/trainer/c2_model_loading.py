...
def align_and_update_state_dicts(model_state_dict, ckpt_state_dict, c2_conversion=True):
    ...
    match_matrix = [len(j) if match(i, j) else 0 for i in model_keys for j in ckpt_keys]  # type: ignore
    ...
    common_prefix = _longest_common_prefix(matched_model_keys)  # type: ignore
    rev_matched_keys = {v: k for k, v in matched_keys.items()}
    original_keys = {k: original_keys[rev_matched_keys[k]] for k in matched_model_keys}

    model_key_groups = _group_keys_by_module(matched_model_keys, original_keys)  # type: ignore
    table = []
    memo = set()
    for key_model in matched_model_keys:
        ...
        if key_model in model_key_groups:
            ...
            table.append((key_model[len(common_prefix) :], key_checkpoint, shape))  # type: ignore
...
def _longest_common_prefix(names: List[str]) -> str:
    ...
    ["abc.zfg", "abc.zef"] -> "abc."
    """
    names = [n.split(".") for n in names]  # type: ignore
