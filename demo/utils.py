...
def create_legend(categories_seen, category_colors, category_map):
    """Create a legend for instance segmentation visualization."""
    legend_elements = []
    for cat_id in categories_seen:
        color = category_colors.get(cat_id, (0, 0, 0, 0.5))
        patch = plt.Rectangle((0, 0), 1, 1, facecolor=color)
        legend_elements.append((patch, category_map.get(cat_id, f"Category {cat_id}")))

    fig = plt.gcf()
    if legend_elements:
        fig.legend(*zip(*legend_elements), loc="center right")
...
def display_detection(path, num_images=9, annotate=True):
    ...
    # Keep track of categories seen for legend
    categories_seen = set()
        ...
        if annotate:
            for ann in coco["annotations"]:
                if ann["image_id"] == img_info["id"]:
                    ...
                    cv2.rectangle(img_array, (x, y), (x + w, y + h), color * 255, 2)
    ...
    # Add legend if annotations are enabled
    if annotate:
        create_legend(categories_seen, category_colors, category_map)

    plt.tight_layout()
    plt.show()


def display_instseg(path, num_images=9, annotate=True, alpha=0.6):
    coco = load_coco_annotations(path)
    img_indices = get_random_image_indices(coco, num_images)
    category_colors = create_category_colors(alpha=alpha)
    axes_flat = setup_plot_grid(num_images)
